...
import com.teEcclesia.identity.data.dataSource.remote.dto.auth.request.LoginRequestDto
...
class AuthenticationRepositoryImpl(
    ...
    override suspend fun login(username: String, password: String) {
        val response = tryToExecute<BaseResponse<AuthenticationResponse>> {
            post(LOGIN_ENDPOINT) {
                setBody(LoginRequestDto(username.trim(), password.trim()))
            }
        }.data ?: throw IllegalStateException("Login failed")
    ...
    override suspend fun refreshAccessToken(): String {
        ...
        }.data ?: throw IllegalStateException("Refresh token failed")
    ...
    companion object {
        ...
        const val REFRESH_ENDPOINT = "api/v1/auth/refresh-token"
